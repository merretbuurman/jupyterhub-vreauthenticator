...
if [ $(id -u) == 0 ] ; then
    ...
    if id jovyan &> /dev/null ; then
        echo "HUHU Set username to: $NB_USER"
    ...
    if [ "$NB_UID" != $(id -u $NB_USER) ] ; then
        echo "HUHU Set $NB_USER UID to: $NB_UID"
        set +e
        usermod -u $NB_UID $NB_USER
        EX=$?
        set -e
        echo "Beginning of workaround."
        if [ $EX -eq 0 ]
        then
          echo "Successfully changed UID of $NB_USER to $NB_UID"
        elif [ $EX -eq 4 ]
        then
          echo "Could not assign UID $NB_UID to user $NB_USER."
          IMPOSTOR=`awk -v val=$NB_UID -F ":" '$3==val{print $1}' /etc/passwd`
          echo "UID $NB_UID is used by $IMPOSTOR. "
          usermod -u 999999 $IMPOSTOR
          echo "$IMPOSTOR now has `id -u $IMPOSTOR`"
          usermod -u $NB_UID $NB_USER
          EX2=$?
          echo "$NB_USER now has `id -u $NB_USER`"
          #exit $EX2
        else
          echo "Could not change UID of $NB_USER (usermod exit code $EX)" >&2
          exit $EX
        fi
